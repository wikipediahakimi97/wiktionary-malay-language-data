...
Before localization, it cleans duplicate code assignments from the Malay baseline by keeping the **first** occurrence and deleting subsequent occurrences. It also repairs alias collisions that would be created by canonical-name replacement.
...
| Path | Purpose |
| --- | --- |
| `sources/ms-old/` | Saved Malay reference, cleaned so later duplicate code assignments are removed. |
| `sources/en-new/` | Untouched fresh English snapshot for the current run. |
| `localized/` | English modules with Malay canonical names and repaired alias collisions. |
| `reports/changes.json` | Detailed duplicate cleanup, canonical-name, and alias changes. |
| `reports/summary.md` | Short workflow summary. |

Both source folders contain `manifest.json` with source revision information. The Malay manifest still records the original downloaded Wikimedia revisions even when duplicate records have been removed from the stored `sources/ms-old/` files during preprocessing.
...
| `malay_duplicate_removal_count` | Number of subsequent duplicate Malay code assignments removed before localization. |
...
For each supported Malay data group, the first `m["code"] = { ... }` assignment is kept and every subsequent assignment of that same code is deleted completely.

The groups are checked independently:

```text
languages                 canonical language modules
language_extras           language /extra modules
etymology_languages       etymology languages/data
families                  families/data
```

Canonical language modules and language `/extra` modules are separate groups because the same code is expected to occur once in each.
...
1. prepares the saved Malay baseline, including any required historical backfill;
